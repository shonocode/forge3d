...
- ~~UV 編集（別 P0 で扱う）~~ → F-M9 で 2D UV エディタ実装済み
- ~~N-gon サポート（quad/tri に正規化）~~ → **half-edge V2（2026-07-21）で quad / n-gon
  を正式サポート**。half-edge を任意アリティ化し、`mesh.metadata.forge3dPolys` で quad 構造を
  永続化。レンダー / 書き出し時のみ扇状三角形分割（`triangulateFaces` + `triToFace`）。
  Tris to Quads / Quads to Tris オペレータ、Extrude / Inset / Bevel / Bridge / Loop Cut の
  quad 出力を含む。詳細は `docs/architecture/forge3d-roadmap.md`「quad 対応 half-edge V2」。
