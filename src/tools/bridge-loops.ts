...
import {
  ...
  faceCreateVerts, faceCalcNormal, faceTriangulate, faceKill, edgeRotate, edgeRotateCheck, loopsOfVert,
...
/** `BM_mesh_beautify_fill` with `VERT_RESTRICT_TAG`, method 1 (angle). */
function beautifyFill(bm: BM, edgeArray: BE[]): void {
  ...
  const cost = (e: BE): number => {
    ...
    const v4 = e.l!.next.v;
    if (v1.tag === v3.tag) return FLT_MAX;
    if (v1 === v3) return FLT_MAX;
    return rotateBeautyAngle(v1.co, v2.co, v3.co, v4.co);
  };
  ...
  while (heap.tree.length) {
    ...
    table[i] = null;
    e = edgeRotate(bm, e);
    if (!e) continue;    states[i]!.add(`${edgePair(e)}|${facePair(e)}`);
    edgeArray[i] = e;
    e.index = i;
